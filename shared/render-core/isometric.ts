...
import { generate3dbarAnimation } from './3dbar-animator';
import {
  ContributionDay,
  ContributionWeek,
} from './contribution';
import { svgTextFontFamily } from './svg-font';
...
export const isometricProcessor = (
  ...
  const scopeId = `r${Math.random().toString(36).slice(2, 10)}`;
  ...
  const gradientArr = gradient
    ? palette.slice(1).map((color, index) => {
        ...
        return sides
          .map((side) => {
            return `<linearGradient id="${scopeId}_gradient_${side}_${index}" gradientTransform="rotate(90)">
  ...
  const cubeBuilder = ({
    ...
    const fillLeft = gradientMode
      ? `url(#${scopeId}_gradient_left_${level - 1})`
      : shadeColorLeft(color);
    const fillRight = gradientMode
      ? `url(#${scopeId}_gradient_right_${level - 1})`
